...
Route::get('/admin/dashboard', function () {
    ...
    return response('<!DOCTYPE html><html><head><title>N.I. CMS Dashboard</title><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>body{font-family:sans-serif;background:#0A192F;color:white;margin:0;padding:2rem}.header{display:flex;justify-content:space-between;align-items:center;padding-bottom:1rem;border-b:2px solid #E53E3E}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1.5rem;margin-top:2rem}.card{background:#1E293B;padding:1.5rem;border-radius:8px;border:1px solid #334155}.num{font-size:2.5rem;font-weight:bold;color:#E53E3E}.btn{display:inline-block;padding:10px 20px;background:#E53E3E;color:white;text-decoration:none;border-radius:4px;font-weight:bold;margin-top:1rem}</style></head><body><div class="header"><h1>N.I. Engineering CMS Admin Dashboard</h1><a href="/api/admin/logout" class="btn">Logout</a></div><div class="grid"><div class="card"><h3>Published Projects</h3><div class="num">'.$projects.'</div></div><div class="card"><h3>Products & Solutions</h3><div class="num">'.$products.'</div></div><div class="card"><h3>Blog Articles</h3><div class="num">'.$posts.'</div></div><div class="card"><h3>Contact Inquiries</h3><div class="num">'.$submissions.'</div></div></div><div style="margin-top:2rem"><a href="/api/admin/deploy" class="btn" style="background:#2563EB">Trigger Static Site Re-Deploy</a></div></body></html>');
});
...
Route::get('/admin/deploy', function () {
    ...
    return response()->json(['status' => 'success', 'message' => 'Deploy webhook triggered']);
});
